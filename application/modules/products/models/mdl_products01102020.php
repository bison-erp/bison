...
class Mdl_Products extends Response_Model
{
    ...
    public function by_product($match,$langue)
    {
        ...
        if($langue=="Arabic"){
            ...
            $this->db->or_like('product_description_en', $match);
        }else{
    ...
    public function calculstockalert($code)
    {
        $this->db->select('ip_products.stock,ip_products.quantite');
        ...
        if (count($query) > 0) {
           ...
           // var_dump($res);
           // return die('gg'.$res.'b1hh'.number_format(($query[0]->stock), 2, '.', '').'hhh2t'.number_format(($restant), 2, '.', ''));
