...
class Ajax extends Admin_Controller
{
    ...
    public function modal_product_lookup()
    {
        ...
        $this->load->model('products/mdl_depot_stock');
        ...
        foreach ($products as $product) {
            ...
            $stock_decl = $this->mdl_stock->where('produit_id', $product->product_id)->get()->result();               
            $products[$i]->stock_decl = $stock_decl;
    ...
    public function process_product_selection()
    {
        ...
        //   return var_dump($this->mdl_declinaison->where('dec_stock_id',88)->get()->result());die('hh');
        // $products = $this->mdl_products->where_in('product_id', $this->input->post('product_ids'))->get()->result();
        $products = array();
     
        //  $products = $this->mdl_products->where_in('product_id', $this->input->post('product_ids'))->get()->result();   
       /* foreach($this->input->post('product_ids') as $j){
           $tab=explode("_",$j); 
           if($tab[1]==0){     
           array_push($products,$this->mdl_products->where('product_id',$tab[0])->get()->result());
           }else{    
            $this->mdl_stock->where('stock_id',$tab[1]);      
            $res_stock=$this->mdl_stock->where('produit_id',$tab[0])->get()->result();   
            array_push($products,$res_stock);
           }
        } */
        //  return var_dump($depot) ;   
    // return var_dump($this->input->post('product_ids')[3]);die('hh');
   // for($j=1;$j<= count($this->input->post('product_ids'));$j++){
      $product_result = array();
      $j = 0;   
    while ($j < count($product_ids)) {   
        $tab=explode("_",$product_ids[$j]);       
        if($tab[1]==0){     
            $products =  $this->mdl_products->where('product_id',$tab[0])->get()->result();
        }else{
            $this->mdl_products->where('product_id',$tab[0]); 
            $this->mdl_stock->where('stock_id',$tab[1]);      
            $products=$this->mdl_stock->where('produit_id',$tab[0])->get()->result();   
        }     
        $i = 0; 
        ...
        foreach ($products as $product) {
                ...
                // $products[$i]->tax_rate_id_dev = $tax_id;
                    ...
                    if(isset($product->id_poduct)){
                        $product->id_poduct = $product->id_poduct;
                    }
                    if(isset($product->name_file)){
                        $product->name_file = $product->name_file;
                    } 
                    if(isset($product->stock_id)){
                    
                        $product->product_sku = $product->ref_stock;
                        $declinaison= $this->mdl_declinaison->where('ip_declinaison.dec_stock_id',$product->stock_id)->get()->result(); 
                        foreach($declinaison as $key => $value){
                            $product->product_description="".$value->name.' :'.$value->valeur.  "\n"  . $product->product_description;
                            $product->product_description_eng= "".$value->name.' :'.$value->valeur. "\n" . $product->product_description_eng;
                            $product->product_description_ar= "".$value->name.' :'.$value->valeur. "\n" . $product->product_description_ar;
                        }
                    
                    }             
                   // $depot_id=$depot[$i];       
                    $product_result[$j]=$product;
                    $i = $i + 1;
        }  
        $j++;  
    }          
...
//        echo "<tr><td colspan='8' style='white-space: initial;'>";
    ...
    public function getProductid()
    {
      ...
      //  return die('hh'.$id);
       ...
       $this->load->model('products/mdl_stock');
        if($dec_id>0){
            $stock_decl = $this->mdl_stock->where('produit_id', $id)->get()->result();  
            return json_encode($stock_decl);         
        }else{
            $this->db->WHERE('product_id', $id);
            $this->db->join('ip_families', 'ip_families.family_id = ip_products.family_id', 'left');
            $prodid = $this->db->get('ip_products')->result();
           
            return json_encode($prodid);         
        }
    }
