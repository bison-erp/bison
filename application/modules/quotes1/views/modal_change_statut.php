...
<div id="modal_change_statut_quote" class="modal col-xs-12" role="dialog" aria-hidden="true"
    ...
    <form class="modal-content">
        <div class="modal-header" style="height: 50px !important;border-bottom: 1px solid #E5E5E5;width: 392px;padding: 15px!important;margin-left: 0;margin-right: 0;border-radius: 6px 6px 0px 0px;">
...
if ($type == 0) {?>
                 <h3 style="font-weight: 600;font-size: 18px;margin:0;"><?php echo lang('quote'); ?> <?php echo lang('status'); ?></h3>
                <?php } else {?>
                 <h3 style="font-weight: 600;font-size: 18px;margin:0;"><?php echo lang('num_facture'); ?> <?php echo lang('status'); ?></h3>
                <?php }?>
        </div>
        <div class="modal-body" style="margin-top: 40px;">
            <div class="row">
                <div class="col-md-12" style=" font-weight: 600;font-size: 15px;text-align: left;">
                    <?php if ($type == 0) {?>
                    <?php echo lang('status_change_of_quote'); ?> #<?php echo $quote->quote_number; ?>
                    <?php } else {?>
                    <?php echo lang('status_change_of_invoice'); ?> #<?php echo $quote->invoice_id; ?>
                    <?php }?>
                </div>
            </div>
...
if ($type == 0) {
    ...
    if ($quote->invoice_status_id == $key) {
        echo "checked";
                                ...
                                data-label_status="<?php echo $value['label'][0] ?>"> <?php
if (($type == 1) && ($value['label'] == "Validé")) {
    ...
    ?>
        ...
        ?>>
                            <?php if ($type == 0) {?>
                            <input type='text' id="data_accepte" class="form-control input-sm datepicker" value="<?php if ($quote->quote_date_accepte != "0000-00-00") {echo date_from_mysql($quote->quote_date_accepte, 1);
        } elseif ($type == 0) {
            if ($quote->quote_status_id != 4) {
                if ($quote->quote_date_accepte != "0000-00-00") {
...
if ($type == 0) {
    echo $quote->quote_status_id;
} else {
    echo $quote->invoice_status_id;
}
?>" data-class_status="<?php
if ($type == 0) {
    echo $quote_statuses[$quote->quote_status_id]['class'];
} else {
...
if ($type == 0) {
    echo $quote_statuses[$quote->quote_status_id]['label'][0];
} else {
...
$(function() {
    ...
    $("#change_status_quote").click(function() {
        ...
        var id_quote = $("#id_quote_to_change_status").val();
        ...
        $.post("<?php echo site_url('quotes/ajax/updateStatutQuote'); ?>", {
            ...
            if ($('#type').val() == 1) {
                if ($('#type').val() == 1 && (id_selected_class.search("approved") == 0)) {
                    $("#line_" + id_quote + " .change_quote_statuses1").html('P');
                    $("#line_" + id_quote + " .change_quote_statuses1").removeClass(
                        id_class_anc);
                    $("#line_" + id_quote + " .change_quote_statuses1").addClass(
                        id_selected_class);
                    ...
                    $("#line_" + id_quote + " .change_quote_statuses1").html(id_selected_label);
                    $("#line_" + id_quote + " .change_quote_statuses1").addClass(
                        id_selected_class);
                    //  alert(id_selected_class);
                    //  alert('facture');
            ...
            if ($('#type').val() == 0) {
                //alert('devis');
                $("#line_" + id_quote + " .change_quote_statuses").html(id_selected_label);
                $("#line_" + id_quote + " .change_quote_statuses").removeClass(id_class_anc);
                $("#line_" + id_quote + " .change_quote_statuses").addClass(id_selected_class);
            }
            $('#modal_change_statut_quote').modal('hide');
