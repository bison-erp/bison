<input type="hidden" value="<?php echo $quote->fabrication_pourcent_remise; ?>" id="quote_pourcent_remise">
<input type="hidden" value="<?php echo $quote->fabrication_montant_remise; ?>" id="quote_montant_remise">
<input type="hidden" value="<?php echo $quote->fabrication_pourcent_acompte; ?>" id="quote_pourcent_acompte">
<input type="hidden" value="<?php echo $quote->fabrication_montant_acompte; ?>" id="quote_montant_acompte">

<input type="hidden" value="<?php echo $quote->timbre_fiscale; ?>" id="timbre_fiscale">


<input type="hidden" value="<?php echo $quote->fabrication_item_subtotal; ?>" id="quote_item_subtotal">
<input type="hidden" value="<?php echo $quote->fabrication_item_tax_total; ?>" id="quote_item_tax_total">
<input type="hidden" value="<?php echo $quote->fabrication_total; ?>" id="quote_total">
<input type="hidden" value="<?php echo $quote->fabrication_total_a_payer; ?>" id="quote_total_a_payer">
...
function addRow(items, init) {
    ...
    if ($('#assigite_tva').val() == 1) {
        $.post("<?php echo site_url('tax_rates/ajax/getTaxRate'); ?>", function(data) {
            ...
            row_add +=
                '<td style="width: 120px;display:none"><input type="hidden" class="input-sm form-control item_price" name="item_price" value="' +
                item_price_value + '" data-id="' + current_id + '" id="item_price_' + current_id + '"></td>';
            row_add +=
                '<td style="width: 120px;display:none"> <input name="subtotal" type="hidden" class="subtotal input-sm form-control" data-id="' +
                current_id + '" id="subtotal_' + current_id + '" value="' + subtotal_value + '"></td>';
            row_add +=
                '<td style="width: 110px;display:none"><select  name="item_tax_rate_id" class="input-sm form-control item_tax_rate_id"  data-id="' +
                current_id + '" id="item_tax_rate_' + current_id + '"> ' + row_add_tax + ' </select></td>';
            row_add +=
                '<td style="width: 120px;display:none"><input name="item_total" class="item_total input-sm form-control" data-id="' +
            ...
            $('#item_table').find('tr.item').find('#item_quantity_' + current_id).keyup(function() {
                changedItemTable($(this), "quantite", "float");
...
$(function() {
    ...
    $('#btn_save_quote, #btn_save_quote1').click(function() {
        ...
        if (items_count > 0) {
            $.post("<?php echo site_url('fabrication/ajax/save'); ?>", {
                ...
                function(data) {
                    ...
                    if (response.success == '1') {
                        window.location = "<?php echo site_url('fabrication/index'); ?>";
                    } else {
                        $("#div_err").empty();
    ...
    $('#btn_generate_pdf').click(function() {
        link = "/fabrication/generate_pdf/" + $('#quoteidd').val();
    ...
    $('#delete_quote').click(function() {
        linkdel = "/fabrication/delete/" + $('#quoteidd').val();
        window.open(linkdel);
    });

    // Recupération des ligne devis
    $.post("<?php echo site_url('fabrication/ajax/getItemsByfabrication'); ?>", {
        quote_id: $('#quoteidd').val()
		...
		<div class="portlet-header">
			<div class="portlet-title align-items-start flex-column">
                ...
                <input type="hidden" id="quoteidd" value="<?php echo $quote_id; ?>">
					<span class="caption-subject bold med-caption dark"><?php echo lang('bf'); ?> #<?php echo $quote->fabrication_number; ?></span>
				</div>
			</div>
...
if ($this->session->userdata["groupes_user_id"] == 1) {
    ?>
                            <li id="delete_quote">
                                <a href="#delete-quote" id="delete_quote" data-quote-id="<?php echo $fabrication_id; ?>"
                                    data-toggle="modal">
...
?>
                        ...
                        <span id="client_infos_edit" style="line-height: 20px;">
							...
							<br>
                            <span><b style="font-size: 12px;font-weight: 600;"><?php echo lang('ste'); ?> :&nbsp;</b><?php echo $quote->client_societe ?></span>
							<br>
                            <span>
                                <span><b style="font-size: 12px;font-weight: 600;"><?php echo lang('address'); ?> :&nbsp;</b></span><?php echo ($quote->client_address_1) ? $quote->client_address_1 : ''; ?>
                        ...
                        ?>
                            <!--div class="col-xs-12 col-md-4">                           
                                <div class="form-group has-info">
                                    <label for="form_control_1"><?php echo lang('setup_choose_language'); ?></label>
                                    <select name="langue" style="padding: 0px 0px;" id="langue"
                                        class="form-control form-control-lg form-control-light" disabled="true">
                                        <option value="">
                                            <?php echo $value; ?>
                                        </option>
                                        <?php foreach (array_filter($tabs) as  $key => $value) {?>
                                        <?php if($quote->langue== $value) {?>
            ...
            <div class="col-md-8">
				<div class="portlet light formulaire no-padding">
					<div class="content-heading">
						<div class="portlet-title left-title">
							<div class="caption font-dark-sunglo">
								<span class="caption-subject bold med-caption dark"><?php echo lang('bf'); ?></span>
							</div>
						</div>
					</div>
                    <div class="row card-row form-row">
                        ...
                        <div class="col-md-4 col-sm-6">
                            <div class="form-group has-info">
                                ...
                                <input id="quote_nature" class="form-control form-control-lg form-control-light"
                                    <?php echo $ett; ?> value="<?php echo $quote->fabrication_nature; ?>">
                                <div class="form-control-focus"> </div>
                            </div>
							<div class="separateur" style="margin-bottom: 25px;"></div>
                        </div>
                        ...
                        <?php
$dateAccepte = $quote->fabrication_date_accepte;
if ($dateAccepte == '') {
    $dateAccepte = date_from_mysql('0000-00-00');
} else {
    $dateAccepte = date_from_mysql($quote->fabrication_date_accepte);
}
?>
                        <div class="col-md-4 col-sm-6">
                            <div class="form-group has-info">
                        ...
                        <div class="col-md-12 col-sm-12">
                            ...
                            <?php }else{ ?>
                                ...
                                <label for="signature">
                                    ...
                                    <span class="box"></span>                                     
                                    <?php echo lang('bf') ?> <?php echo lang('signature') ?>
                                </label>
                            </div>
							<div class="separateur" style="margin-bottom: 25px;"></div>
                        </div>
   ...
   <div class="portlet light profile no-shabow bg-light-blue">
    <div id="fact_prod">
		...
		<div class="portlet-body">
			<div class="row card-row form-row">
                    <div class="col-md-12">
                            <?php $this->layout->load_view('fabrication/partial_item_table');?>
                            <div style="clear:both"></div>
                            <div class="form-group has-info">
                                <label for="form_control_1"><?php echo lang('notes'); ?></label>
                                <textarea height="40" name="notes" id="notes"
                                    style="width: 100%;padding: 5px 10px;" class="form-control form-control-lg form-control-light"><?php echo $quote->notes; ?></textarea>
                                <div class="form-control-focus"></div>
                            </div>
                            <div style="clear:both"></div>
                    </div>
