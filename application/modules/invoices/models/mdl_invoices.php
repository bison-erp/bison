...
class Mdl_Invoices extends Response_Model
{
    ...
    public function default_select()
    {
        $this->db->select("
			...
			IFNULL(ip_invoice_amounts.timbre_fiscale, '0.00') AS timbre_fiscale,
			...
			IFNULL(ip_invoice_amounts.invoice_balance, '0.00') AS invoice_balance,
                        ...
                        IFNULL(ip_invoice_amounts.invoice_pourcent_remise, '0') AS invoice_pourcent_remise,
                        IFNULL(ip_invoice_amounts.invoice_montant_remise, '0.00') AS invoice_montant_remise,
                        IFNULL(ip_invoice_amounts.invoice_pourcent_acompte, '0') AS invoice_pourcent_acompte,
                        IFNULL(ip_invoice_amounts.invoice_montant_acompte, '0.00') AS invoice_montant_acompte,
                        IFNULL(ip_invoice_amounts.invoice_item_subtotal_final, '0.00') AS invoice_item_subtotal_final,
                        IFNULL(ip_invoice_amounts.invoice_item_tax_total_final, '0.00') AS invoice_item_tax_total_final,
...
// $this->load->model('quote_rappel/mdl_quote_rappel');
        ...
        $this->db->join('ip_quote_date_rappel', 'ip_quote_date_rappel.rappel_qote_id = ip_invoices.invoice_id', 'left');
    ...
    public function default_join()
    {
        ...
        $this->db->join('ip_delai_paiement', 'ip_invoices.invoice_delai_paiement = ip_delai_paiement.delai_paiement_id', 'left');
...
//            'invoice_group_id' => array(
            ...
            'depot_invoice' => array('field' => 'depot_invoice'),
        );
    }
...
//function get_by_id($id){
    //
    //               // $this->db->select('ip_invoices');
    //        $this->db->where('ip_invoices.invoice_id', $id);
    //return($this->db->get('ip_invoices'));
    //    }
    ...
    public function getTypepdf()
    {
        ...
        WHERE `setting_key` LIKE 'pdf_invoice_template' AND `setting_value` LIKE 'avec entête'");
        $result = $query->result();
        $var = 0;
        if ($result) {
            return $var;
