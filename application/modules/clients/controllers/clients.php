...
class Clients extends Admin_Controller
{
    ...
    public function form($id = null)
    {
        ...
        if ($sess_cont_add == 1) {
            ...
            $this->layout->set('contact_type', $this->mdl_clients->form_value('contact_type') ?: 0);
    ...
    {  
        ...
        if ($sess_cont_add == 1) {
            ...
            $this->db->WHERE("$where");
            $this->db->WHERE("ip_invoices.avoir",0);
            $this->db->WHERE("ip_invoices.recursive",0);
          ...
          //  var_dump($invoices_total_amounts_db);
            ...
            $where = "ip_clients.client_id = $client_id";
            $this->db->WHERE("$where");
          //  $this->db->WHERE("ip_invoices.invoice_status_id <> 7");
          $this->db->WHERE("ip_invoices.avoir",0);
          ...
          //  $this->db->WHERE("ip_invoices.recursive",0);
            ...
            if (!empty($result_credit)) {
                foreach ($result_credit as $credit) {
                   ...
                   // $data_payment["avoir"] = $credit->avoir;
            ...
            $this->db->WHERE("ip_commande.client_id",$client_id);
            ...
            $this->db->group_by("ip_devises.devise_id");
            $commande_total_amounts_db = $this->db->get("ip_commande")->result();
            $commande_total_amounts = array();
            if (!empty($commande_total_amounts_db)) {
                foreach ($commande_total_amounts_db as $amount) {
                    $commande_total_amounts[$amount->devise_id]["quotes_sum_subtotal"] = $amount->quotes_sum_subtotal;
                    $commande_total_amounts[$amount->devise_id]["quotes_sum_total"] = $amount->quotes_sum_total;
                    $commande_total_amounts[$amount->devise_id]["count"] = $amount->count;
                }
            }
         ...
         //   $this->db->order_by(TRIM($order_by), $order_method);
            $commande_table = $this->db->get("ip_commande")->result();
        ...
        $this->db->group_by("ip_devises.devise_id");
        $this->db->WHERE("ip_bl.client_id",$client_id);
        $bl_tot_total_amounts_db = $this->db->get("ip_bl")->result();
        $bl_tot_total_amounts = array();
        if (!empty($bl_tot_total_amounts_db)) {
            foreach ($bl_tot_total_amounts_db as $amount) {
                $bl_tot_total_amounts[$amount->devise_id]["quotes_sum_subtotal"] = $amount->quotes_sum_subtotal;
                $bl_tot_total_amounts[$amount->devise_id]["quotes_sum_total"] = $amount->quotes_sum_total;
                $bl_tot_total_amounts[$amount->devise_id]["count"] = $amount->count;
            }
        }
        ...
        $this->db->select("*,(case when trim(ip_clients.client_societe) = '' then ip_clients.client_name else ip_clients.client_societe end) as client_societe2");
        $this->db->WHERE("ip_bl.client_id",$client_id);     
        $bl_tot = $this->db->get("ip_bl")->result();
...
//                        'logs' => $this->mdl_activites->order_by("activites_date_created", "desc")->where('client_id', $client_id)->get()->result(),
                    ...
                    'commandes_total_amounts'=>$commande_total_amounts,
                    'bl_tot_total_amounts'=>$bl_tot_total_amounts,
                    'documents' => $documents,
                    'commandes' => $commande_table,
                    'bl_tot' => $bl_tot,
                   // 'bl_tot' => $this->mdl_bl->where('ip_bl.client_id', $client_id)->get()->result(),
                   // 'commande_table' => $this->mdl_commande->where('ip_commande.client_id', $client_id)->get()->result(),
                )
            );

            $this->layout->buffer(
                array(
                    array('commande_table', 'commande/partial_commande_table'),
                    array('livraison_table', 'bl/partial_bl_table'),
                    array('invoice_table', 'invoices/partial_invoice_table'),
                    array('invoice_have_table', 'invoices/partial_have_invoice_table'),
