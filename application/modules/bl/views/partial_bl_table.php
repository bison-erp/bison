...
foreach ($bl_tot as $quote) {
    $nb_rap = 0;
    $nb_ra = 0;
    $i = 0;
    $quote_id = $quote->bl_id;
    $a = explode('//', @$arrayHasDates[$i]);

//    if (@$rappels) {
    //
    //        foreach ($rappels as $rappel) {
    //            if (($quote->bl_id == $rappel->rappel_object_id) && ($rappel->rappel_status == 1)) {
    //                $nb_rap+=1;
    //            }
    //        }
    //        foreach ($rappels as $rappel) {
    //            if ($quote->bl_id == $rappel->rappel_object_id) {
    //                $nb_ra+=1;
    //            }
    //        }
    //    }
    ?>
<tr id="line_<?php echo $quote->bl_id; ?>">

    <td>
        <a href="<?php echo site_url('bl/view/' . $quote->bl_id); ?>" title="<?php echo lang('edit'); ?>">
            <?php echo $quote->bl_number; ?>
        </a>
    </td>
     <td style="text-align:center;">
        <span style="width: 75% !important;  cursor:pointer;"
            data-quote-id="<?php echo $quote->bl_id; ?>"
            class="change_bl_statuses label <?php echo $quote_statuses[$quote->bl_status_id]['class']; ?>">
            <?php echo $quote_statuses[$quote->bl_status_id]['label'][0] ?>
        </span>
    </td>
...
if ($quote->client_societe != '') {
    ...
    <td style="width:100px;">
        <span title="<?php echo $quote->bl_nature; ?>">
            <?php
$nat = $quote->bl_nature;
    ...
    <td><?php
echo format_devise($quote->bl_item_subtotal_final, $quote->devise_id);
    ?></td>
    <td><?php
echo format_devise($quote->bl_total_final, $quote->devise_id);
    ?></td>
    <td><?php echo substr($quote->delai_paiement_label, 0, -14); ?></td>
    <td><?php echo substr($quote->user_name, 0, 10);
    if (strlen($quote->user_name) > 10) {
        echo '<b style="color:#27871e">&nbsp;...</b>';
    }
    ?></td>
    <td style="text-align:center;"><?php if (($quote->bl_status_id != 5) && ($quote->bl_status_id != 4)) {
        $data_send_mail = 1;
    } else {
        $data_send_mail = 0;
    }?>
        <div class="md-checkbox">
            <input type="checkbox" name="Choix[]" data-send_mail="<?php echo $data_send_mail; ?>"
                value="<?php echo $quote->bl_id; ?>" id="checkbox30_<?php echo $quote->bl_id; ?>" class="md-check"
                onchange="myFunctionchech(<?php echo $quote->bl_id; ?>)">
            <label for="checkbox30_<?php echo $quote->bl_id; ?>" <?php if ($data_send_mail == 0) {
    ...
    <td>
        <div class="options btn-group">
            <a class="btn btn-sm btn-default dropdown-toggle option_btn" data-toggle="dropdown"
                data-id="<?php echo $quote->bl_id; ?>" href="#">
                <i class="fa fa-cog"></i> <?php //echo lang('options');      ?>
            </a>
            <ul class="dropdown-menu dropdown-btngroup-menu">
                <?php if ($sess_devis_add == 1) {?> <li>
                    <a href="<?php echo site_url('bl/view/' . $quote->bl_id); ?>">
                        <i class="fa fa-edit fa-margin"></i> <?php echo lang('edit'); ?>
                    </a>
                </li>
                <?php }?>
                <li>
                    <a href="<?php echo site_url('bl/generate_pdf/' . $quote->bl_id); ?>" target="_blank">
                        <i class="fa fa-file-pdf-o  fa-margin"></i> <?php echo lang('download_pdf'); ?>
                    </a>
                </li>
                <?php if (($quote->bl_status_id != 5) && ($quote->bl_status_id != 4)) {?>
                <li>
                ...
                <li><a style="cursor: pointer;">
                        <input type="text" style="width: 100%;position: absolute;opacity: 0;cursor: pointer;"
                            class="simpliest-usage" id="multi_date_pick_<?php echo $quote->bl_id; ?>" />
                        <span class="calendar_a" id="span_pick_<?php echo $quote->bl_id; ?>">
                ...
                <li>
                    <a class="calendar_a supp_rappel" id="supp_rappel_<?php echo $quote->bl_id; ?>" style="<?php ?>"
                        data-id="<?php echo $quote->bl_id; ?>"><i class="fa fa-trash-o fa-margin"></i>
                ...
                <li>
                    <a href="<?php echo site_url('bl/delete/' . $quote->bl_id); ?>"
                        onclick="return confirm('<?php echo lang('delete_bl_warning'); ?>');">
...
foreach ($devises as $devise) {
        $bl_tot_total_amounts[$devise->devise_id]["quotes_sum_subtotal"] = isset($bl_tot_total_amounts[$devise->devise_id]["quotes_sum_subtotal"]) ? $bl_tot_total_amounts[$devise->devise_id]["quotes_sum_subtotal"] : 0;
        $bl_tot_total_amounts[$devise->devise_id]["quotes_sum_total"] = isset($bl_tot_total_amounts[$devise->devise_id]["quotes_sum_total"]) ? $bl_tot_total_amounts[$devise->devise_id]["quotes_sum_total"] : 0;
        $bl_tot_total_amounts[$devise->devise_id]["count"] = isset($bl_tot_total_amounts[$devise->devise_id]["count"]) ? $bl_tot_total_amounts[$devise->devise_id]["count"] : "0";
        ?>
<tr style="background: #E1F0FF; font-weight:bold;">
    <td colspan="6" style=" border:none; padding: 12px 8px;">
        <?php echo (int) $bl_tot_total_amounts[$devise->devise_id]["count"] . " (" . $devise->devise_symbole . ")"; ?>
    </td>
    <td colspan="2" style=" border:none; padding: 12px 15px; text-align: right;">
        <?php echo format_devise($bl_tot_total_amounts[$devise->devise_id]["quotes_sum_subtotal"], $devise->devise_id); ?>
    </td>
    <td colspan="2" style=" border:none; padding: 12px 15px; text-align: right;">
        <?php echo format_devise($bl_tot_total_amounts[$devise->devise_id]["quotes_sum_total"], $devise->devise_id); ?>
    </td>
...
$start_line = isset($start_line) ? $start_line : 1;?>
    ...
    $(function() {
        ...
        $('.change_bl_statuses').click(function() {
            //alert('hh');
            $('#modal-placeholder').load(
                "<?php echo site_url('bl/ajax/modal_change_statut/0'); ?>", {
                    bl_id: $(this).data('quote-id')
                });
        });
    ...
    function myFunctionchech(e) {
        ...
        if ($("#check_all").is(':checked')) {
            if (myInteger > 0) {
                ...
                $('#group_bl_invoice').prop("disabled", false);
            }
        } else {
            if (myInteger > 0) {
                $('#relance_rappel').prop("disabled", false);
