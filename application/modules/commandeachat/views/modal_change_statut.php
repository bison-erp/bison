...
if ($type == 0) {
        if ($commande->commande_status_id == $key) {
            echo "checked";
        }} else {
...
if (($type == 1) && ($value['label'] == "Validé")) {
    ...
    ?>
        ...
        ?>>
                            <?php if ($type == 0) {?>
                            <input type='text' id="data_accepte" class="form-control form-control-md form-control-light datepicker" value="<?php if ($commande->commande_date_accepte != "0000-00-00") {echo date_from_mysql($commande->commande_date_accepte, 1);
        } elseif ($type == 0) {
            if ($commande->commande_status_id != 4) {
                if ($commande->commande_date_accepte != "0000-00-00") {
                    echo date($commande->commande_date_accepte);
                } else {
                    echo date("d/m/Y");
                }
            } else {
            ...
            ?>" style="z-index: 10051 !important;">
                            <?php } else {?>
                            <input type='text' id="data_accepte" class="form-control form-control-md form-control-light input-sm datepicker"
                                value="<?php echo date("d/m/Y"); ?>" style="z-index: 10051 !important;">
...
if ($type == 0) {
    echo $commande->commande_status_id;
} else {
    echo $quote->invoice_status_id;
}
?>" data-class_status="<?php
if ($type == 0) {
    echo $quote_statuses[$commande->commande_status_id]['class'];
} else {
...
if ($type == 0) {
    echo $quote_statuses[$commande->commande_status_id]['label'][0];
} else {
...
$(function() {
    ...
    $("#change_status_quote").click(function() {
        ...
        var id_commande = $("#id_quote_to_change_status").val();
        ...
        $.post("<?php echo site_url('commande/ajax/updateStatutCommande'); ?>", {
            ...
            if ($('#type').val() == 1) {
                if ($('#type').val() == 1 && (id_selected_class.search("approved") == 0)) {
                    $("#line_" + id_commande + " .change_quote_statuses1").html('P');
                    $("#line_" + id_commande + " .change_quote_statuses1").removeClass(
                        id_class_anc);
                    $("#line_" + id_commande + " .change_quote_statuses1").addClass(
                        id_selected_class);
                    ...
                    $("#line_" + id_commande + " .change_quote_statuses1").html(
                        id_selected_label);
                    $("#line_" + id_commande + " .change_quote_statuses1").addClass(
                        id_selected_class);
                    //  alert(id_selected_class);
            ...
            if ($('#type').val() == 0) {
                //alert('devis');
                $("#line_" + id_commande + " .change_commande_statuses").html(id_selected_label);
                $("#line_" + id_commande + " .change_commande_statuses").removeClass(id_class_anc);
                $("#line_" + id_commande + " .change_commande_statuses").addClass(
                    id_selected_class);
            }
            $('#modal_change_statut_quote').modal('hide');
