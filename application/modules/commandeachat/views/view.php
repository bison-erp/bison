<input type="hidden" value="<?php echo $commande->commande_achats_pourcent_remise; ?>" id="quote_pourcent_remise">
<input type="hidden" value="<?php echo $commande->commande_achats_montant_remise; ?>" id="quote_montant_remise">
<input type="hidden" value="<?php echo $commande->commande_achats_pourcent_acompte; ?>" id="quote_pourcent_acompte">
<input type="hidden" value="<?php echo $commande->commande_achats_montant_acompte; ?>" id="quote_montant_acompte">

<input type="hidden" value="<?php echo $commande->timbre_fiscale; ?>" id="timbre_fiscale">


<input type="hidden" value="<?php echo $commande->commande_achats_item_subtotal; ?>" id="quote_item_subtotal">
<input type="hidden" value="<?php echo $commande->commande_achats_item_tax_total; ?>" id="quote_item_tax_total">
<input type="hidden" value="<?php echo $commande->commande_achats_total; ?>" id="quote_total">
<input type="hidden" value="<?php echo $commande->commande_achats_total_a_payer; ?>" id="quote_total_a_payer">
<input type="hidden" value="<?php echo $commande->langue; ?>" id="langue">
...
$(function() {
    ...
    $('#btn_save_quote, #btn_save_quote1').click(function() {
        ...
        if (items_count > 0) {
            $.post("<?php echo site_url('commandeachat/ajax/save'); ?>", {
                    commanded_id: $('#commanded').val(),
                    fournisseur_id: $('#fournisseur_id').val(),
                    commande_achat_number: $('#quote_number').val(),
                    commande_achat_date_created: $('#quote_date_created').val(),
                    commande_achat_date_expires: $('#quote_date_expires').val(),
                    commande_achat_status_id: $('#quote_status_id').val(),
                //  commande_achats_password: $('#quote_password').val(),
                // commande_date_accepte: $('#quote_date_accepte').val(),
                    commande_achat_nature: $('#commande_achat_nature').val(),
                    commande_achats_delai_paiement: $('#quote_delai_paiement').val(),
                    notes: $('#notes').val(),
                ...
                function(data) {
                    ...
                    if (response.success == '1') {
                        window.location = "<?php echo site_url('commandeachat/index'); ?>";
                    } else {
                        $("#div_err").empty();
    ...
    $('#btn_generate_pdf').click(function() {
        link = "/commandeachat/generate_pdf/" + $('#commanded').val();
    ...
    $('#delete_quote').click(function() {
        linkdel = "/commandeachat/delete/" + $('#commanded').val();
        window.open(linkdel);
    });

    // Recupération des ligne devis
    $.post("<?php echo site_url('commandeachat/ajax/getItemsByCommande'); ?>", {
        commanded_id: $('#commanded').val()
    ...
    $('#search_fournisseur').click(function() {
...
<input type="hidden" id="devise" value="<?php echo $commande->devise_id; ?>">
<input type="hidden" id="symbole_devise" value="<?php echo $commande->devise_symbole; ?>">

<input type="hidden" id="tax_rate_decimal_places" value="<?php echo $commande->number_decimal; ?>">
<input type="hidden" id="currency_symbol_placement" value="<?php echo $commande->symbole_placement; ?>">
<input type="hidden" id="thousands_separator" value="<?php echo $commande->thousands_separator; ?>">
<input type="hidden" id="decimal_point" value="." />

<?php
if (($commande->commande_achat_status_id == 4) || ($commande->commande_achat_status_id == 5) || ($commande->commande_achat_status_id == 6)) {
    $ett = 'disabled';
} else {
    $ett = '';
}
	...
	<div class="portlet light profile no-shabow bg-light-blue">
		<div class="portlet-header">
			<div class="portlet-title align-items-start flex-column">
                ...
                <input type="hidden" id="commanded" value="<?php echo $commande->commande_achat_id; ?>">
					<span class="caption-subject bold med-caption dark"><?php echo lang('bons_commande'); ?> #<?php echo $commande->commande_achat_number; ?></span>
				</div>
			</div>
				...
				<div class="pull-right btn-group">
                    <div class="options btn-group pull-left">
                        ...
                        <ul class="dropdown-menu">                           
                            <li>
                                <a href="#" id="btn_generate_pdf" data-quote-id="<?php echo $commande->commande_achat_id; ?>">
                                    <i class="fa fa-print fa-margin"></i>
                                    <?php echo lang('download_pdf'); ?>
                                </a>
                            </li>
                            <li>
                                <a href="<?php echo site_url('mailer/commandeachat/' . $commande->commande_achat_id); ?>">
...
if ($this->session->userdata["groupes_user_id"] == 1) {
    ?>
                            <li id="delete_quote">
                                <a href="#delete-quote" id="delete_quote" data-quote-id="<?php echo $commande->commande_achat_id; ?>"
                                    data-toggle="modal">
					...
					<div class="portlet light formulaire no-padding">
						<div class="content-heading">
							<div class="portlet-title left-title">
								<div class="caption font-dark-sunglo">
									<span class="caption-subject bold med-caption dark"><?php echo lang('fournisseur'); ?></span>
								</div>
							</div>
						</div>
						...
						<div class="col-md-10">
                        <?php
/*if ($commande->fournisseur_titre == 0) {
    $fournisseur_titre = 'M.';
} elseif ($commande->fournisseur_titre == 1) {
    $fournisseur_titre = 'Mme.';
} elseif ($commande->fournisseur_titre == 2) {
    $fournisseur_titre = 'Melle.';
}*/
?>

                        <input type="hidden" name="fournisseur" id="fournisseur_id" value="<?php echo $commande->id_fournisseur; ?>">
                        <input type="hidden" name="fournisseur_name" id="fournisseur_name"
                            value="<?php echo $commande->raison_social_fournisseur; ?>">
                        <input type="hidden" name="devise" id="devise" value="<?php echo $commande->devise_id; ?>">
                        <span id="fournisseur_infos_edit" style="line-height: 20px;">
							<h4 class="clt_name">
                            <a
                                href="<?php echo site_url('fournisseurs/form/' . $commande->id_fournisseur); ?>"><?php echo $commande->raison_social_fournisseur ; ?></a>
							</h4>
                            <br>
                            <span><b style="font-size: 12px;font-weight: 600;"><?php echo lang('ste'); ?> :&nbsp;</b><?php echo $commande->raison_social_fournisseur ?></span>
                            
                            <br>
                            <span><b style="font-size: 12px;font-weight: 600;"><?php echo lang('n_telephone'); ?> :&nbsp;</b>
                                <?php echo $commande->tel_fournisseur ?>

                                <?php
if (strlen($commande->tel2_fournisseur) > 0) {?>
                                - <?php echo $commande->tel2_fournisseur ?>
                                <?php }?></span>
                            <br>
                           </span>
                    </div>
                    <div class="col-md-2">
                        <button id="search_fournisseur" type="button" class="btn btn-success">
                    ...
                    <div class="row card-row form-row">
                        <div class="col-md-4 col-sm-6">
                            <div class="form-group has-info">
                                <label for="form_control_1"><?php echo lang('bon_commande'); ?></label>
                                <input readonly="readonly" id="quote_number" class="form-control form-control-lg form-control-light"
                                    value="<?php echo $commande->commande_achat_number; ?>">
                        ...
                        <div class="col-md-4 col-sm-6">
                            <div class="form-group has-info">
                                ...
                                <input id="commande_achat_nature"  class="form-control form-control-lg form-control-light"
                                    <?php echo $ett; ?> value="<?php echo $commande->commande_achat_nature; ?>">
                                <div class="form-control-focus"> </div>
                            </div>
							<div class="separateur" style="margin-bottom: 25px;"></div>
                        </div>
                        <div class="col-md-4 col-sm-6">
                            <div class="form-group has-info">
                                ...
                                <select name="commande_achat_status_id" id="quote_status_id" class="form-control form-control-lg form-control-light">
                                    <?php foreach ($quote_statuses as $key => $status) {?>
                                    <option value="<?php echo $key; ?>"
                                        <?php if ($key == $commande->commande_achat_status_id) {?>selected="selected" <?php }?>>
                                ...
                                <div class="quote-properties has-feedback">
                                    ...
                                    <div class="input-group">
                                        <input name="quote_date_created" style="z-index: 1 !important;"
                                            ...
                                            value="<?php echo date_from_mysql($commande->commande_achat_date_created); ?>" disabled>
                            ...
                            <div class="form-group has-info">
                                ...
                                <div class="input-group">
                                    <input name="quote_date_expires" <?php echo $ett; ?> style="z-index: 1 !important;"
                                        ...
                                        value="<?php echo date_from_mysql($commande->commande_achat_date_expires); ?>">
                                    <span class="input-group-addon">
                                        <i class="fa fa-calendar fa-fw"></i>
                                    </span>
                                </div>
                    ...
                    <div class="col-md-12">
                            ...
                            <div class="form-group has-info">
                                ...
                                <textarea height="40" name="notes" id="notes"
                                    style="width: 100%;padding: 5px 10px;height:80px;" class="form-control"><?php echo $commande->notes; ?></textarea>
                                <div class="form-control-focus"></div>
                            </div>
                            <div style="clear:both"></div>
                    </div>
