...
class newaccount extends Base_Controller
{
    ...
    {  
        ...
        $query="
        INSERT INTO `oussema_erpbison`.`ab_abonnes` (`nom`, `prenom`, `societe`, `email`, `adresse`, `complement`, `ville`, `code_postal`, `pays`, `telephone`, `mobile`, `fax`, `site_web`, `matricule_fiscale`, `registre_commerce`, `statut`, `created`, `database`, `licence_key`, `pack_id`,`id_userwhmcs`)
       ...
       $passwordtocken = $this->generate_password($password, $user_psalt);
        ...
        $query5 = "UPDATE  `oussema_erpbison`.`ip_settings` SET `setting_value`= '" . $footer_pdf_defaut . "' WHERE `setting_key`='pdf_invoice_footer'";
      ...
      //  $from_email_def = array($settings['noreplay_mail'], $settings['from_name']);             
        ...
        $message = "
          ...
          //  phpmail_send($from_email_def, $to, $subject, $message);
    ...
    {
        $query5 = "UPDATE `oussema_erpbison`.`ab_abonnes` SET `statut` = '" . $statut . "' WHERE `ab_abonnes`.`abonne_id` =" . $id;
        $this->db->query($query5);    
    }
    ...
    public function existLicenceKey($licence)
    {
        ...
        return  var_dump(count($this->db->query($query)));
        if (count($this->db->query($query))>0) {
            return 1;
        } else {
            return 0;
        }
