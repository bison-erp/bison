...
class cronrappelautomatique extends Base_Controller
{
    ...
    public function send_mail1($db, $tab, $taille)
    {
        ...
        if ($taille > 0) {
            ...
            if ($tab[$taille - 1]['type'] == 'i') {
                ...
                // return var_dump($ip_quotes);
                $from .= $ip_quotes[0]->user_email;

                //    return var_dump($usermailconnect);
                ...
                // return print_r($message.'hh'.$subject);die();
                $this->delrappel($db, $tab[$taille - 1]['ip_rappelmail_id']);
    ...
    public function mailenvoi($db, $from, $to, $subject, $message, $licence, $files)
    {
        $smtp = $this->getSmtp($db);
        //  return print_r('hh'.$from.'hh'.$to);die();

        $from = "oussematest0@gmail.com";
        $from = "oussematest0@gmail.com";
        $to = "oussematest0@gmail.com";
        // echo('hh'.$from.'hh'.$to);
        //    $headers = "From:$from";
        //    $headers .= "Reply-To:$smtp";
        //  $headers .= "CC:$from";

        /* $email="oussematest0@gmail.com";
        $headers = "From: <".$email.">\n";
        $headers .= "Reply-To: ".$email."\n";*/

        //  return die('hh'.$from);die();
        $headers = "From:" . $from;
        $headers .= "Reply-To:noreply@novatis.tn\r\n";
    ...
    public function mailenvoisendphpmailer($db, $from, $to, $subject, $message, $licence, $attachment_path)
    {
        ...
        // return var_dump('hh'.$from);
        //  $from="ahmedamine@novatis.tn";
        //   $to="oussematest0@gmail.com";
        phpmail_send($from, $to, $subject, $message, $attachment_path, $cc = null, $bcc = null);
    ...
    {
        $this->db->where("setting_key", 'smtp_username');
        $result = $this->db->get($db . ".ip_settings")->result();
        return $result[0]->setting_value;
    }

    /* public function setupdate($db)
{
$CI = &get_instance();
...
$data = array(
'email_template_title' => 'Relance automatique devis',
);

$CI->db->where(trim("ip_email_templates.email_template_title"),$c);
$CI->db->update($db . ".ip_email_templates", $data);
